...
done_processing:
  ...
  uint64_t extra = 0;
  uint64_t extraLimit = 10;

  while (trace_count < max_cycles) {
    if (Verilated::gotFinish() || (done_reset && (dtm->done() || jtag->done() || tile->io_success)))
      break;
  ...
  if (Verilated::gotFinish()) {
    fprintf(stderr, "*** FAILED *** via finish (seed %d) after %ld cycles\n", random_seed, trace_count);
    ret = 1;
  }
  ...
  {
    fprintf(stderr, "*** PASSED *** via timeout (timeout, seed %d) after %ld cycles\n", random_seed, trace_count);
    ret = 0;
  }
